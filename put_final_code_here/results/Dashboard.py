...
with st.sidebar:
    ...
    # Dropdown menu to select Bundesland 
    ...
    # Conditionally filter Wohnflaeche (WFL) based on Construction Year selection
...
if st.button('Map'):
    ...
    if 'df_filtered' in st.session_state and not st.session_state['df_filtered'].empty:
        ...
        st.markdown(f'<h3 style="color: black;">The Risk Score for the selected filters is: {mean_probability:.2f}%</h3>', unsafe_allow_html=True)
        st.markdown(f'<h4 style="color: black; font-size: 16px;">Number of used properties: {used_properties_count}</h4>', unsafe_allow_html=True)
        
        # Format the text for the legend with colors specified directly
        properties_count_text = (
            f"(0-5%: {count_0_5} <span style='color:#8BC34A;'>lightgreen</span>, "
            f"5-15%: {count_5_10} <span style='color:#4CAF50;'>green</span>, "
            f"15-25%: {count_10_15} <span style='color:#CDDC39;'>darkgreen</span>, "
            f"25-50%: {count_15_20} <span style='color:#FF5722;'>red</span>, "
            f"Over 50%: {count_over_75} <span style='color:#B71C1C;'>darkred</span>)"
        )
        ...
        def get_marker_color(probability):
            ...
            # Probability range: >20% to 25% -> orange
                ...
                return ('red')   # '#FF5722' -> orange RGBA
            else:
                return ('darkred')   # '#B71C1C' -> darkred RGBA
        ...
        legend_html = """
            <div class="legend-container">
                ...
                <div class="legend">
                    ...
                    <div class="legend-item">
                        <div style="background-color: red;"></div> >25% - 50%
                    </div>
                    <div class="legend-item">
                        <div style="background-color: darkred;"></div> >50%
                    </div>
                </div>
            </div>
        """
